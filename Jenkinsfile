...
pipeline {
    ...
    environment {
        // 1. 代码仓库配置
        CODEUP_REPO_URL = "https://github.com/1980744819/TrendRadar.git"
        ...
        PROJECT = "trendradar"
        IMAGE_REPO = "${REPO_ADDR}/${PROJECT}/${PROJECT}"  // 私人镜像仓库地址+镜像名（替换为你的实际地址）
        MCP_IMAGE_REPO = "${REPO_ADDR}/${PROJECT}/${PROJECT}-mcp"  // MCP 镜像仓库地址
        NAMESPACE = "prod"
        GIT_CREDENTIAL_ID = "credential_github"  // GitHub 凭证 ID
        HARBOR_CREDENTIAL_ID = "credential_harbor_admin"  // Harbor 凭证 ID
        
        // 3. Helm 配置
        HELM_CHART_PATH = "./trendradar-helm"  // Helm chart 路径
        ...
        stage('Init Image Variables') {
            steps {
                script {
                    ...
                    MCP_FULL_IMAGE_NAME = "${MCP_IMAGE_REPO}:${IMAGE_TAG}"
                    
                    // （可选）将变量注入全局环境，后续阶段可直接使用
                    env.IMAGE_TAG = IMAGE_TAG
                    env.FULL_IMAGE_NAME = FULL_IMAGE_NAME
                    env.MCP_FULL_IMAGE_NAME = MCP_FULL_IMAGE_NAME
                    
                    echo "初始化镜像变量完成："
                    echo "镜像标签：${IMAGE_TAG}"
                    echo "完整镜像名：${FULL_IMAGE_NAME}"
                    echo "MCP完整镜像名：${MCP_FULL_IMAGE_NAME}"
                }
            }
        }
        ...
        stage('Build Docker Image') {
            steps {
                container('podman') {
                    ...
                    sh """
                        set -e
                        podman build -f ${DOCKERFILE_PATH}/Dockerfile \
                        --squash --network=host \
                        --tls-verify=false \
                        -t ${FULL_IMAGE_NAME} \
                        .
                    ...
                    echo "开始构建 MCP 镜像 ${MCP_FULL_IMAGE_NAME}..."
                    // 构建 MCP 镜像
                    sh """
                        set -e
                        podman build -f ${DOCKERFILE_PATH}/Dockerfile.mcp \
                        --squash --network=host \
                        --tls-verify=false \
                        -t ${MCP_FULL_IMAGE_NAME} \
                        .
                    """
                    echo "MCP 镜像构建完成！"
                }
                ...
                container('podman') {
                    withCredentials([usernamePassword(credentialsId: "${HARBOR_CREDENTIAL_ID}", usernameVariable: 'HARBOR_USER', passwordVariable: 'HARBOR_PASS')]) {
                        sh '''
                            ...
                            # 推送 MCP 镜像
                            echo "推送 MCP 镜像 ${MCP_FULL_IMAGE_NAME} 到 Harbor..."
                            podman push --tls-verify=false "${MCP_FULL_IMAGE_NAME}"
                            echo "MCP 镜像推送完成！"
                        '''
                    }
                }
    ...
    post {
        success {
            script {
                container('podman') {
                    echo "流水线构建成功，已推送镜像 ${FULL_IMAGE_NAME} 和 ${MCP_FULL_IMAGE_NAME} 到私人仓库并完成部署！"
                    // 清理本地构建的镜像
                    echo "开始清理本地构建的镜像..."
                    sh """
                        podman rmi ${FULL_IMAGE_NAME} || true
                        podman rmi ${MCP_FULL_IMAGE_NAME} || true
                    """
                    echo "本地镜像清理完成！"
                }
            }
